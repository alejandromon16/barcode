...
interface TicketStatsTableProps {
  generatedByStats?: TicketStats[] | null | [];
  width: number;
}

const TicketStatsTable: React.FC<TicketStatsTableProps> = ({ generatedByStats = [], width }) => {
  const tableHead = ['Nombre Completo', 'T. Generados'];
        ...
        <View>
          ...
          <ScrollView style={styles.dataWrapper}>
            <Table borderStyle={{ borderTopWidth: 1, borderColor: '#C1C0B9', }}>
              {generatedByStats &&
                generatedByStats.map((stats, index) => (
                  <Row
                    key={index}
